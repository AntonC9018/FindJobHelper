...
public static class ParserHelper
{
    ...
    public record struct SkipToStartOfLastSegmentResult(
        bool IsOnlySegment,
        bool IsEmpty,
        bool IsEmptySegment,
        ParserPosition PreviousSegmentEnd);

    public static SkipToStartOfLastSegmentResult SkipToStartOfLastSegment(
        this ref Parser parser,
        string separator)
    {
        if (parser.IsEmpty)
        ...
        var previousSegmentEnd = parser.Position;
        while (true)
        {
            var bparser = parser.BufferedView();
            ...
            if (result.EndOfInput)
            {
                return new()
                {
                    IsOnlySegment = isFirstSegment,
                    PreviousSegmentEnd = previousSegmentEnd,
                };
            }
            isFirstSegment = false;
            parser.MoveTo(bparser.Position);
            previousSegmentEnd = parser.Position;
            parser.Move(separator.Length);
            if (parser.IsEmpty)
            {
                return new()
                {
